...
exports.restoreHomeTutor = async (req, res) => {
  try {
    ...
    if (
      tutor.deletedThrough === "Instructor" ||
      tutor.deletedThrough === "ByUpdation"
    ) {
      return res.status(400).send({
        success: true,
...
exports.restoreHTutorServiceArea = async (req, res) => {
  try {
    ...
    if (
      area.deletedThrough === "Instructor" ||
      area.deletedThrough === "ByUpdation"
    ) {
      return res.status(400).send({
        success: true,
...
exports.restoreHTutorPrice = async (req, res) => {
  try {
    ...
    if (!price) {
      return res.status(400).send({
        success: false,
        message: "This home tutor price is not present in delete section!",
      });
    }
    if (
      price.deletedThrough === "Instructor" ||
      price.deletedThrough === "ByUpdation"
    ) {
      return res.status(400).send({
        success: true,
        message: "Warning! This home tutor price is not deleted by Swasti!",
      });
    }
    await price.update({ deletedThrough: null });
    // Restore area
    await price.restore();
    // Final Response
    res.status(200).send({
      success: true,
      message: "Home tutor price restored successfully!",
    });
  } catch (err) {
    res.status(500).send({
      success: false,
...
exports.restoreHTutorTimeSlote = async (req, res) => {
  try {
    ...
    if (
      slote.deletedThrough === "Instructor" ||
      slote.deletedThrough === "ByUpdation"
    ) {
      return res.status(400).send({
        success: true,
...
exports.restoreHTutorImage = async (req, res) => {
  try {
    ...
    if (
      image.deletedThrough === "Instructor" ||
      image.deletedThrough === "ByUpdation"
    ) {
      return res.status(400).send({
        success: true,
    ...
    const images = await HTutorImages.count({
      where: { homeTutorId: image.homeTutorId, deletedThrough: null },
    });
    if (parseInt(images) < 3) {
      ...
      res.status(400).send({
        success: false,
        message:
          "Home tutor image can not restore! There are already 3 image present",
      });
    }
  } catch (err) {
    res.status(500).send({
