...
exports.restoreCoupon = async (req, res) => {
  try {
    ...
    if (
      coupon.deletedThrough === "Instructor" ||
      coupon.deletedThrough === "ByUpdation"
    ) {
      return res.status(400).send({
        success: true,
...
exports.getAllInstructorCoupon = async (req, res) => {
  try {
    ...
    // Search
    const condition = [{ createrId: req.user.id }];
    if (approval) {
      condition.push({
        approvalStatusByAdmin: approval, // Pending, Approved, Rejected
      });
  ...
  try {
    ...
    if (req.user) {
      conditionForCoupon = {
        id: couponId,
        approvalStatusByAdmin: "Approved",
        [Op.or]: [{ createrId: req.user.id }, { creater: "Admin" }],
      };
      conditionForCourse = {
        id: courseId,
        approvalStatusByAdmin: "Approved",
    ...
    if (req.user) {
      ...
      if (!isAdded) {
        ...
        if (softDelete) {
          if (
            softDelete.deletedThrough === "Admin" ||
            softDelete.deletedThrough === "ByUpdation"
          ) {
            return res.status(400).send({
              success: false,
      ...
      if (!isAdded) {
        ...
        if (softDelete) {
          if (
            softDelete.deletedThrough === "Instructor" ||
            softDelete.deletedThrough === "ByUpdation"
          ) {
            return res.status(400).send({
              success: false,
  ...
  try {
    ...
    if (isCourseHasCoupon.validTill) {
      const isCouponExpired =
        new Date().getTime() > parseInt(isCourseHasCoupon.validTill);
      if (isCouponExpired) {
        return res.status(400).send({
          success: false,
          message: `This coupon has expired!`,
    ...
    const savedMoney =
      (parseInt(course.coursePrice) * parseInt(coupon.discountInPercent)) / 100;
